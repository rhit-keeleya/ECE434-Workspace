...
static unsigned int gpioLED2 = 115;	///< hard coding the LED2 gpio to P9_27 (GPIO115)
static unsigned int gpioButton = 60;   ///< hard coding the button gpio for this example to P9_12 (GPIO60)
static unsigned int gpioButton2 = 66;	///< hard coding the button2 gpio to P8_07 (GPIO66)
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   printk(KERN_INFO "GPIO_TEST: The first button state is currently: %d\n", gpio_get_value(gpioButton));

   // GPIO numbers and IRQ numbers are not the same! This function performs the mapping for us
   irqNumber = gpio_to_irq(gpioButton);
   printk(KERN_INFO "GPIO_TEST: The first button is mapped to IRQ: %d\n", irqNumber);

   // This next call requests an interrupt line
   result = request_irq(irqNumber,             // The interrupt number requested
                        (irq_handler_t) ebbgpio_irq_handler, // The pointer to the handler function below
                        IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING,   // Interrupt on rising edge (button press, not release) - edit: both
                        "ebb_gpio_handler",    // Used in /proc/interrupts to identify the owner
                        NULL);                 // The *dev_id for shared interrupt lines, NULL is okay

   printk(KERN_INFO "GPIO_TEST: The first interrupt request result is: %d\n", result);
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   gpio_export(gpioButton2, false);          // Causes gpio115 to appear in /sys/class/gpio
			                    // the bool argument prevents the direction from being changed
   // Perform a quick test to see that the button is working as expected on LKM load
   printk(KERN_INFO "GPIO_TEST: The second button state is currently: %d\n", gpio_get_value(gpioButton));

   // GPIO numbers and IRQ numbers are not the same! This function performs the mapping for us
   irqNumber2 = gpio_to_irq(gpioButton2);
   printk(KERN_INFO "GPIO_TEST: The second button is mapped to IRQ: %d\n", irqNumber);
   ...
   printk(KERN_INFO "GPIO_TEST: The second interrupt request result is: %d\n", result);
   return result;
}
